...
const getContactById = async (req, res) => {
  try {
    ...
    if (!ObjectId.isValid(id)) {
      return res.status(400).json({ message: '[controllers/contacts] Invalid id format.' });
    }
    ...
    if (!contact) {
      return res.status(404).json({ message: '[controllers/contacts] Contact not found.' });
    }

    return res.status(200).json(contact);
  } catch (error) {
    console.error('[controllers/contacts] Error fetching contact by id:', error);
    return res.status(500).json({ message: '[controllers/contacts] Server error.' });
  }
};
